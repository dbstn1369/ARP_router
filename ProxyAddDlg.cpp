...
CProxyAddDlg::CProxyAddDlg(CWnd* pParent)
...
void CProxyAddDlg::OnSelchangeComboDevice() 
{
	...
	unsigned char *addr = SystemEtherInfo[cb_device.GetCurSel( )].MacAddr;
	...
	for( int i = 0; i < 4; i++ ) 
	{
		memcpy( c_temp, SystemEtherInfo[cb_device.GetCurSel( )].IPAddr + i*4, 3 );	
		c_temp[3] = '\0';
		ip_Addr[i] = atoi( c_temp );
	}
	
	m_IPAddr.SetAddress( ip_Addr[0], ip_Addr[1], ip_Addr[2], ip_Addr[3] );
